...
from pgzero.spellcheck import distance
import pygame as pg
...
def draw():
    ...
    screen.clear()
    screen.bilt("dungeon", (0, 0))
    if game_over:
        screen.draw.text("GAME OVER!", fontsize=60,
                         center=CENTRE, color=FONT_COLOUR)
    elif game_complete:
        screen.draw.text("You Won!", fontsize=60,
                         center=CENTRE, color=FONT_COLOUR)
    else:
        hero.draw()
        draw_lairs(lairs)
...
def draw_counters(eggs_collected, lives):
    ...
    screen.draw.text(str(eggs_collected), fontsize=40,
                     pos=(30, HEIGHT - 30), color=FONT_COLOUR)
    screen.blit("life-count", (60, HEIGHT - 30))
    screen.draw.text(str(lives), fontsize=40, pos=(
        90, HEIGHT - 30), color=FONT_COLOUR)
    screen.draw.text(str(lives), fontsize=40, pos=(
        90, HEIGHT - 30), color=FONT_COLOUR)
